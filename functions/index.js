...
const EXISTING_POPUP_FIX_VERSION = 'smart-agent-v1-2026-05-18';
...
async function handleIncomingText(from, rawText, metadata = {}) {
  try {
    return await handleSmartIncomingText(from, rawText, metadata);
  } catch (err) {
    console.error("Smart agent failed - falling back to legacy bot:", getErrorPayload(err));
    return handleLegacyIncomingText(from, rawText, metadata);
  }
}

async function handleSmartIncomingText(from, rawText, metadata = {}) {
...
async function smartNormalizeText({ text, business, session, step }) {
  const local = localSmartNormalize(text, session, step);
  ...
  const context = buildSmartAgentContext({ business, session, step });
  const prompt = [
    "אתה שכבת הבנה לסוכן וואטסאפ של מערכת קביעת תורים.",
    "המטרה שלך: להמיר הודעת לקוח חופשית לטקסט קצר שהבוט הקיים יודע לבצע.",
    "אל תענה ללקוח. תחזיר JSON בלבד.",
    "אם הלקוח רוצה לקבוע תור החזר normalizedText=\"1\".",
    "אם הלקוח רוצה לבטל תור החזר normalizedText=\"2\".",
    "אם הלקוח שואל על שעות פתיחה החזר normalizedText=\"3\".",
    "אם הוא מבקש חזרה/איפוס החזר normalizedText=\"0\".",
    "בשלב בחירת יום/שעה/שירות/תור לביטול: בחר את מספר האפשרות המתאימה מתוך האפשרויות בלבד.",
    "אם אין התאמה בטוחה, החזר את ההודעה המקורית.",
    "בשלב ask_name החזר את השם כפי שנכתב, בלי שינוי.",
    "פורמט חובה: {\"normalizedText\":\"...\",\"confidence\":0.0}",
    "",
    context,
    `הודעת הלקוח: ${text}`,
  ].join("\n");
  ...
  try {
    ...
    if (normalized && confidence >= 0.55) return normalized;
  } catch (err) {
    console.error("OpenAI normalization failed:", getErrorPayload(err));
  }

  return text;
}

function buildSmartAgentContext({ business, session, step }) {
  const lines = [];
  lines.push(`עסק: ${business.businessName || business.name || "העסק"}`);
  lines.push(`שלב נוכחי: ${step}`);

  if (step === "choose_day" && Array.isArray(session.days)) {
    lines.push("אפשרויות ימים:");
    session.days.forEach((d, i) => lines.push(`${i + 1}. ${d.label || ""} ${d.pretty || ""} ${d.date || ""}`));
  }

  if (step === "choose_time" && Array.isArray(session.times)) {
    lines.push(`תאריך נבחר: ${session.selectedDateLabel || session.selectedDate || ""}`);
    lines.push("אפשרויות שעות:");
    session.times.forEach((t, i) => lines.push(`${i + 1}. ${t}`));
  }

  if (step === "choose_service" && Array.isArray(session.services)) {
    lines.push("אפשרויות שירותים:");
    session.services.forEach((s, i) => lines.push(`${i + 1}. ${s.label || s.value || s.text || "שירות"}`));
  }

  if (step === "cancel_select" && Array.isArray(session.cancelOptions)) {
    lines.push("אפשרויות תורים לביטול:");
    session.cancelOptions.forEach((a, i) => lines.push(`${i + 1}. ${formatDatePrettyFromKey(a.date)} ${a.time || ""} ${a.service || ""}`));
  }

  if (step === "cancel_confirm") {
    lines.push("אפשרויות אישור ביטול: 1=כן לבטל, 2=לא לבטל");
  }

  return lines.join("\n");
}

function localSmartNormalize(text, session, step) {
  const clean = cleanText(text);
  const lowered = clean.toLowerCase();

  if (!clean) return "";
  if (/^(0|חזור|חזרה|תפריט|ביטול פעולה|התחל מחדש|איפוס)$/i.test(clean)) return "0";
  if (step === "ask_name") return clean;

  if (step === "main_menu") {
    if (clean === "1" || /קבע|לקבוע|תור|פנוי|זמין|זמינות|מחר|היום|שעה/.test(lowered)) return "1";
    if (clean === "2" || /בטל|ביטול|לבטל|התור שלי/.test(lowered)) return "2";
    if (clean === "3" || /שעות|פתיחה|סגור|פתוח|מתי/.test(lowered)) return "3";
  }
  ...
  if (step === "choose_time" && Array.isArray(session.times)) {
    ...
    if (timeMatch) {
      ...
      const minute = String(timeMatch[2] || "00").padStart(2, "0");
      const normalizedTime = `${hour}:${minute}`;
      const index = session.times.findIndex((t) => String(t) === normalizedTime);
      if (index >= 0) return String(index + 1);
    }
  }

  if (["choose_day", "choose_time", "choose_service", "cancel_select", "cancel_confirm"].includes(step)) {
...
function isResetText(text) {
  return ["התחל", "התחל מחדש", "menu", "תפריט", "איפוס"].includes(String(text || "").trim().toLowerCase());
}
...
function getOpenAIClient(apiKey) {
  try {
    ...
    console.error("OpenAI package is missing. Run: npm install openai inside the functions folder.", getErrorPayload(err));
    return null;
  }
}
